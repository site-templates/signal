...
<section id="page-header" class="relative pt-44 pb-4 sm:pt-52">
    ...
    <div class="relative mx-auto w-full max-w-3xl px-6 text-center">
        @if ($showIcon)
        <span class="enter-2 mx-auto flex size-12 items-center justify-center rounded-2xl bg-ink shadow-lg shadow-ink/20" data-enter>
        ...
        @endif
        <p class="enter-3 mt-6 font-mono text-[11px] font-medium tracking-widest text-accent uppercase" data-enter>{{ $eyebrow }}</p>
        <h1 class="font-display enter-4 mt-4 text-4xl font-bold tracking-tight text-balance sm:text-6xl" data-enter>{{ $heading }}</h1>
        @if ($body)
        <p class="enter-5 mx-auto mt-5 max-w-xl text-lg/8 text-pretty text-muted" data-enter>{{ $body }}</p>
        @endif
    </div>
</section>
